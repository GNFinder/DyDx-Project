...
interface ISoloMargin {
    ...
    function ownerSetInterestSetter(uint marketId, address interestSetter)
        external;
    ...
    function getMarketPriceOracle(uint marketId)
        external
        view
        returns (address);

    function getMarketInterestSetter(uint marketId)
        external
        view
        returns (address);
    ...
    function getNumExcessTokens(uint marketId)
        external
        view
        returns (Types.Wei memory);
    ...
    function getMarket(uint marketId)
        external
        view
...
contract DydxFlashloanBase {
    ...
    function _getRepaymentAmountInternal(uint amount)
        internal
        pure
        returns (uint)
    {
...
contract dYdXFlashLoanTest is DydxFlashloanBase {
    ...
    function initiateFLashLoan(address _token, uint256 _amount) public {
